...
static void next_token(void) { curtok = lexer_next(); }

static int peek_type(void) { return lexer_peek()->type; }

static int accept(token_type_t type) {
...
static void expect(token_type_t type) {
  if (sh.parse_error)
    return;
  if (!accept(type)) {
    char got[128]; // flawfinder: ignore
    token_desc(lexer_peek(), got, sizeof(got));
    sh_syntax("expected '%s', got %s", token_name(type), got);
  }
}
...
static int is_command_start(token_type_t t) {
  switch (t) {
  case TOK_WORD:
  case TOK_ASSIGNMENT:
  case TOK_IO_NUMBER:
...
static int is_redirect(token_type_t t) {
  switch (t) {
  case TOK_LESS:
  case TOK_GREAT:
  case TOK_DLESS:
...
static struct node *p_simple_command(void) {
  ...
  for (;;) {
    ...
    if (t == TOK_ASSIGNMENT && !have_word) {
      ...
      if (eq[1]) {
        value = ast_word();
        word_append_part(value, ast_wordpart(WPART_LITERAL, eq + 1));
      }
      assign_list_append(&assigns_head, ast_assignment(name, value));
    } else if (is_redirect(t)) {
      struct redirect *r = p_redirect();
      if (r)
        redir_list_append(&redirs_head, r);
    } else if (t == TOK_WORD || t == TOK_ASSIGNMENT) {
      next_token();
      have_word = 1;
...
static struct node *p_if_clause(void) {
...
static struct node *p_while_clause(void) {
...
static struct node *p_until_clause(void) {
  struct node *cond, *body;
  struct redirect *redirs;
...
static struct node *p_for_clause(void) {
  const char *var;
...
static struct node *p_case_clause(void) {
  struct word *subject;
...
static struct node *p_brace_group(void) {
  struct node *body;
  struct redirect *redirs;
...
static struct node *p_subshell(void) {
  struct node *body;
  struct redirect *redirs;
...
static struct node *p_compound_command(void) {
  ...
  case TOK_WHILE:
    next_token();
    return p_while_clause();
  case TOK_UNTIL:
    next_token();
    return p_until_clause();
  case TOK_FOR:
    next_token();
    return p_for_clause();
  case TOK_CASE:
    next_token();
    return p_case_clause();
  case TOK_LBRACE:
    next_token();
    return p_brace_group();
  case TOK_LPAREN:
    next_token();
    return p_subshell();
  default:
...
static struct node *try_function_def(void) {
...
static struct node *p_command(void) {
  ...
  if (t == TOK_IF || t == TOK_WHILE || t == TOK_UNTIL || t == TOK_FOR ||
      t == TOK_CASE || t == TOK_LBRACE || t == TOK_LPAREN)
