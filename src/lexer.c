...
void
lexer_set_prompts(const char *ps1, const char *ps2)
{
	ps1_str = ps1;
	ps2_str = ps2;
}
...
void
lexer_set_alias(int enable)
{
	alias_enabled = enable;
}

const char *
token_name(token_type_t t)
{
...
static struct token *
make_token(token_type_t type, const char *value)
{
	struct token *t;
...
static void
read_pending_heredocs(void)
{
	struct heredoc_pending *hd, *next;

	for (hd = pending_heredocs; hd; hd = next) {
		next = hd->next;
		hd->redir->heredoc_body = lexer_read_heredoc(
		    hd->delim, hd->strip_tabs, hd->quoted);
...
static struct token *
classify_word(struct token *tok)
{
	token_type_t rw;
