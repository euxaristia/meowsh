...
/* newgrp [group] */
int
builtin_newgrp(int argc, char **argv)
{
	(void)argc;
	(void)argv;
...
builtin_ulimit(int argc, char **argv)
{
	...
	while (i < argc && argv[i][0] == '-') {
		switch (argv[i][1]) {
		case 'f': resource_type = RLIMIT_FSIZE; break;
		case 'c': resource_type = RLIMIT_CORE; break;
		case 'd': resource_type = RLIMIT_DATA; break;
		case 'n': resource_type = RLIMIT_NOFILE; break;
		case 's': resource_type = RLIMIT_STACK; break;
		case 't': resource_type = RLIMIT_CPU; break;
		case 'v':
#ifdef RLIMIT_AS
