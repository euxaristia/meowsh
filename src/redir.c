...
static struct saved_fd *save_fd(int fd, struct saved_fd *list) {
  struct saved_fd *sf;
  int saved;
...
struct saved_fd *redir_apply(struct redirect *redirs) {
  ...
  for (r = redirs; r; r = r->next) {
    ...
    case REDIR_HEREDOC_STRIP: {
      ...
      if (r->heredoc_quoted)
        body = sh_strdup(r->heredoc_body ? r->heredoc_body : "");
      else
        body = expand_heredoc(r->heredoc_body ? r->heredoc_body : "");
