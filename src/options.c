...
static const struct optmap optmap[] = {
    {'a', "allexport", OPT_ALLEXPORT},     {'e', "errexit", OPT_ERREXIT},
    {'f', "noglob", OPT_NOGLOB},           {'h', "hashall", OPT_HASHALL},
    {'i', "interactive", OPT_INTERACTIVE}, {'m', "monitor", OPT_MONITOR},
    {'n', "noexec", OPT_NOEXEC},           {'u', "nounset", OPT_NOUNSET},
    {'v', "verbose", OPT_VERBOSE},         {'x', "xtrace", OPT_XTRACE},
    {'C', "noclobber", OPT_NOCLOBBER},     {0, NULL, 0}};

static int option_set_flag(char c, int set) {
...
int option_is_set(unsigned int flag) { return (sh.opts & flag) != 0; }
...
static int option_by_name(const char *name, int set) {
  const struct optmap *om;
      ...
      /* Normal option flags */
      {
        ...
        for (p = argv[i] + 1; *p; p++) {
          if (option_set_flag(*p, set) < 0) {
            ...
            sh_error("illegal option %c%c", argv[i][0], *p);
          }
        }
      }
    } else {
