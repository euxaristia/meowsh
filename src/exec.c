...
static void
hash_insert(const char *name, const char *path)
{
	unsigned int h = hash_string(name);
	struct hash_entry *he;
...
void
find_command(const char *name, struct cmd_entry *entry)
{
...
static void
apply_assigns(struct assignment *assigns, int persist)
{
	struct assignment *a;

	for (a = assigns; a; a = a->next) {
		char *value = a->value ?
		    expand_word(a->value, 0) : sh_strdup("");
...
static struct saved_assign *
temp_assigns(struct assignment *assigns)
{
...
static void
restore_assigns(struct saved_assign *saved)
{
...
static int
exec_function(struct node *func_body, int argc, char **argv,
...
static void
register_function(const char *name, struct node *body)
{
	unsigned int h = hash_string(name);
	struct func_entry *fe;
...
int
exec_simple_cmd(struct node *n, int flags)
{
	char **argv;
	int argc;
	...
	case CMD_FUNCTION:
		{
			...
			status = exec_function(entry.u.func, argc, argv,
			    n->data.simple.redirs);
	...
	/* Check errexit */
	if (option_is_set(OPT_ERREXIT) && status != 0 &&
...
void
exec_subshell(struct node *body, struct redirect *redirs, int flags)
{
	pid_t pid;
...
/* Main dispatch */
int
exec_node(struct node *n, int flags)
{
	int status = 0;
	...
	case NODE_CASE:
		{
			...
			for (item = n->data.case_cmd.items; item;
				...
				for (pat = item->data.case_item.patterns; pat;
				     pat = pat->next) {
