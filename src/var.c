...
int
var_set(const char *name, const char *value, int export)
{
	unsigned int h;
	struct var_entry *v;
...
int
var_readonly(const char *name, const char *value)
{
	struct var_entry *v;
...
void
var_set_posparams(int argc, char **argv)
{
	struct posparams *pp;
	int i;
...
void
var_push_posparams(int argc, char **argv)
{
	struct posparams *pp;
	int i;
...
var_shift(int n)
{
	...
	memmove(pp->argv, pp->argv + n,
	    (size_t)(pp->argc - n + 1) * sizeof(char *));
	pp->argc -= n;
	return 0;
}
...
var_special(int c)
{
	...
	case '#':
		snprintf(buf, sizeof(buf), "%d",
		    sh.posparams ? sh.posparams->argc : 0);
		return buf;
	case '0':
...
void
var_walk_exports(void (*fn)(const char *name, const char *value))
{
	int i;
	struct var_entry *v;
